...
def write_arff_csv(dataset, class1, class2, class_top):
    logger("Start write arff")
    final_op_set = []
    opcode_bank = {}
    index_helper_x = 0
    ...
    for item in class1:
        for opc_i in final_op_set:
            for opc_j in final_op_set:
                ...
                if (len([item_t for item_t in class_top if item_t[0] == key])==1):
                    if key in item:
                        dict_mal[key]=str(item[str(opc_i) + " " + str(opc_j)])
                    else:
                        dict_mal[key]="0"
        dict_mal["label"]="1"
    ...
    for item in class2:
        for opc_i in final_op_set:
            for opc_j in final_op_set:
                ...
                if (len([item_t for item_t in class_top if item_t[0] == key])==1):
                    if key in item:
                        dict_bin[key]=str(item[str(opc_i) + " " + str(opc_j)])
                    else:
                        dict_bin[key]="0"
        dict_bin["label"]="0"

    with open(dataset, mode='w') as csv_file:
        fieldnames = fields
        writer = csv.DictWriter(csv_file, fieldnames=fields)
        writer.writeheader()
        writer.writerow(dict_mal)
        writer.writerow(dict_bin)
...
def opcode_sequence_generator(repo, dump_method_dir):
    ...
    for top_count in range(1,10):
        ...
        write_arff_csv(repo + 'result'+str(top_count*10)+'.csv', mal_class, bin_class,mal_class_sum+bin_class_sum)
